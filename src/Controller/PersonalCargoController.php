...
class PersonalCargoController extends Controller
{
    ...
    /**
     * @Route("/organigrama", name="PersonalCargo_listar2", methods={"GET"})
     */
    ...
    {   
        ...
        try
        {
            ...
            $cargo = $this->getDoctrine()->getRepository(PersonalCargo::class)->findBy(array('estado' => '1'));
            $docderiv = $this->getDoctrine()->getRepository(DocProcRevision::class)->findBy(array('fkresponsable' => $s_user['id'], 'firma' => 'Por firmar', 'estado' => '1'));
            $fcaprobjf = $this->getDoctrine()->getRepository(FichaCargo::class)->findBy(array('fkjefeaprobador' => $s_user['id'], 'firmajefe' => 'Por aprobar', 'estado' => '1'));
            $fcaprobgr = $this->getDoctrine()->getRepository(FichaCargo::class)->findBy(array('fkgerenteaprobador' => $s_user['id'], 'firmagerente' => 'Por aprobar', 'estado' => '1'));
            $Personas = $this->getDoctrine()->getRepository(Personal::class)->findBy(array('estado' => '1', 'fkprocesoscargo' => null));

            return $this->render('personalcargo/organigrama.html.twig', array('organigrama' => $data4, 'cargo' => $cargo, 'dominio'=>$_SERVER['HTTP_HOST'], 'cantidad' => $data5, 'personas'=>$Personas, 'parents' => $parent, 'children' => $child, 'permisos' => $permisos, 'docderiv' => $docderiv, 'fcaprobjf' => $fcaprobjf, 'fcaprobgr' => $fcaprobgr, 'aux' => $Personal));
        }catch(Exception $e){
    ...
    /**
     * @Route("/personalcargo_filtrar", methods={"POST"}, name="PersonalCargo_filtrar")
     */
    public function filtrar()
    {
        try {
            ...
            while(!empty($aux)){
                ...
                $child = $this->get_children($lista, $item);
            ...
            if(isset($dataorg[0])){
                $dataorg[0]['parent'] = null;
            }

            $serializer = new Serializer(array(new GetSetMethodNormalizer()), array('json' => new JsonEncoder()));
            $json = $serializer->serialize($dataorg, 'json');
            $resultado = array('response'=>$json,'success' => true,
                'message' => 'Organigrama del personal listado correctamente.');
            $resultado = json_encode($resultado);
            return new Response($resultado);
        } catch (Exception $e) {
            echo 'Excepción capturada: ',  $e->getMessage(), "\n";
    ...
    public function get_children($array, $id){
        $children = [];
        $aux = $array;
        foreach ($aux as $ditem){
            if($ditem['parent'] == $id) array_push($children, $ditem['key']);
        }
        return $children;
    }
    ...
    {   
        ...
        try
        {
            ...
            return $this->render('personalcargo/organigrama.html.twig', array('organigrama' => $data4, 'cargo' => $cargo, 'dominio'=>$_SERVER['HTTP_HOST'], 'cantidad' => $data5, 'personas'=>$Personas, 'parents' => $parent, 'children' => $child, 'permisos' => $permisos, 'docderiv' => $docderiv, 'fcaprobjf' => $fcaprobjf, 'fcaprobgr' => $fcaprobgr, 'aux' => $dataorg));
        }catch(Exception $e){
            echo 'Excepción capturada: ',  $e->getMessage(), "\n";
        }
    }
